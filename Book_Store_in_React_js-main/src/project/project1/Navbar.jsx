...
const Navbar = () => {
  ...
  return (
    ...
    <section
      className={`lg:px-32  p-1 bg-pink scrollbar-corner-rose-500 fixed top-0 left-0 w-full z-30 ${
    ...
    >
     ...
     <button className="w-1/2 p-2 bg-pink-400 text-white rounded-full" >Login</button>
      </div>
      ...
      <nav className="flex justify-between px-3 items-center p-1">
        ...
        <div className="flex  items-center relative">
          ...
          >
            {link.map((link, i) => (
              <a
                ...
                key={i}
                className={`hover:bg-pink-300 md:text-[14px]  ${
                  i == I ? "lg:border-b-2 border-pink-500" : ""
                }`}
              >
                {link.name}
              </a>
          ...
          <button onClick={()=>setOpen(!open)} className="ml-10 py-2 px-5 text-pink font-semibold rounded-full bg-pink-800 h-fit  ">
            Sign in
          </button>
        </div>
      </nav>
